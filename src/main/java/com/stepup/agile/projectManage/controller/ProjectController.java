...
//         model.addAttribute("jsonList3", JSONArray.fromObject(subTaskList));
         ...
         model.addAttribute("alertmsg", "프로젝트 조회에 실패하셨네요?!  ^^");
         model.addAttribute("url", "showProjectMain.pj");
         return "common/alert";
      }
   }
   ...
   public String insertProject(Model model, @ModelAttribute("loginUser") Member m, ProjectHistory projectHistory, Project project) {
      ...
      if(project.getProjectCode() != 0) {
         ...
         //project insert fail
         //alert 페이지로 보내기
         model.addAttribute("alertmsg", "프로젝트 생성에 실패하셨네요?!  ^^");
         model.addAttribute("url", "showProjectMain.pj");
         return "common/alert";
   ...
   public String updateProjectOne(Model model, @ModelAttribute("loginUser") Member m, ProjectHistory projectHistory, String updatePossible) {
      if(updatePossible.equals("ok")) {
         ...
         if(result > 0) {
            ...
            //alert 페이지로 보내기
            model.addAttribute("alertmsg", "프로젝트 수정에 실패하셨네요?!  ^^");
            model.addAttribute("url", "showProjectMain.pj");
            return "common/alert";
         }
      }else {
         //alert 페이지로 보내기
         model.addAttribute("alertmsg", "수정권한이 없는데 수정을 하셨군요?! 마스터만 수정이 가능합니다 ^^");
         model.addAttribute("url", "showProjectMain.pj");
         return "common/alert";
      ...
      if(userTeamCode != 0) {
         ...
         if(userProject == null) {
            ...
            if(result > 0) {
               ...
               model.addAttribute("alertmsg", "프로그램 오류로 멤버 추가 실패 하였습니다. 관리자에게 연락해주세요!? ^^");
               model.addAttribute("url", "showProjectMain.pj");
               return "common/alert";
            }
         //중복일 때   
            ...
            //alert 페이지로 보내기
            model.addAttribute("alertmsg", "기존 멤버군요?! 멤버 추가 실패하였습니다. ^^");
            model.addAttribute("url", "showProjectMain.pj");
            return "common/alert";
         }
      }else {
         //alert 페이지로 보내기
         model.addAttribute("alertmsg", "팀원명 오류로 멤버 추가 실패하였습니다. 팀원명을 확인해주세요 ?! ^^");
         model.addAttribute("url", "showProjectMain.pj");
         return "common/alert";
      }
   }   
